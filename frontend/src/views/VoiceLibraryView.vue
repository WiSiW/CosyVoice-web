...
import { voiceAudioUrl } from '@/api/voices'
...
import type { PreparedAudio, Voice } from '@/types'
...
async function submit(): Promise<void> {
  ...
  try {
    const voice = await voices.create({
      ...
      autoRegister: settings.state.autoRegister && Boolean(draft.promptText.trim()),
    })
...
<template>
  <div class="page-header">
    <div>
      <h1>音色库</h1>
      <p>
        上传或录制 3~30 秒参考音频即可创建专属音色，创建后可在合成页的「3s 极速复刻 / 跨语种复刻」中直接选用。
      </p>
    </div>
    ...
    <div class="grid two">
      <div>
        ...
        <div class="field">
          ...
          />
          <span class="sub">填写后可在「3s 极速复刻」与「自然语言控制」中直接使用该音色。</span>
        </div>
